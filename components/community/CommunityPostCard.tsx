...
export interface CommunityPostData {
  ...
  replyCount: number;
  authorName: string;
  authorPathway: string | null;
}

export function CommunityPostCard({ post }: { post: CommunityPostData }) {
  const initial = post.authorName[0];
  return (
    <Card className="flex items-start gap-4">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-1 flex flex-wrap items-center gap-2 text-caption text-text-muted">
          ...
          {post.authorPathway && <Badge tone="neutral">{post.authorPathway}</Badge>}
